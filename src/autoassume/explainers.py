# ===========================================
# Module: Explanations of Assumption Checks
# Author: Kenneth Leung
# Last Modified: 31 Dec 2021
# ===========================================
# Reference: https://people.duke.edu/~rnau/testing.htm

# ---------------------------------
#   Overview - Linear Regression
# ----------------------------------
explain_description_linear_regression = """
    There are a number of assumptions that justify the use of linear regression models for purposes 
    of inference or prediction.

    (1) Linearity of relationship between dependent (target) and independent (predictor) variables

    (2) Statistical independence of errors (in particular, no correlation between consecutive 
    errors (aka autcorrelation) in the case of time series data)

    (3) Homoscedasticity (constant variance) of the errors

    (4) Normality of error distribution

    (5) No multicollinearity between independent (predictor) variables

    If any of these assumptions is violated (i.e., if there are nonlinear relationships between 
    dependent and independent variables or the errors exhibit correlation, heteroscedasticity, 
    or non-normality), then the forecasts, confidence intervals, and scientific insights yielded 
    by a regression model may be (at best) inefficient or (at worst) seriously biased or misleading.

    """


# ---------------------
#   Homoscedasticity
# ---------------------
explain_description_homosced = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Dudfsis aute irure dolor in reprehenderit
    """

explain_solution_homosced = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit
    """

explain_plot_residual_homosced = """
    Lorem ipsum dolor sdvxbfit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris
    """


# ---------------------
#    Independence
# ---------------------
explain_description_independence = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit
    """

explain_solution_independence = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit
    """

explain_plot_acf = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris
    """


# ---------------------
#      Linearity
# ---------------------
explain_description_linearity = """
    Violations of linearity or additivity are extremely serious: \
    If you fit a linear model to data which are nonlinearly or nonadditively related, \
    your predictions are likely to be seriously in error, especially when you extrapolate beyond \
    the range of the sample data.
    """

explain_solution_linearity = """
    Consider applying a nonlinear transformation to the dependent and/or independent variables if you can think of a \
    transformation that seems appropriate. (Don't just make something up!) For example, if the data are strictly positive, \
    the log transformation is an option. (The logarithm base does not matter--all log functions are same up to linear \
    scaling--although the natural log is usually preferred because small changes in the natural log are equivalent to percentage \
    changes.)
    """

explain_plot_residual_linearity = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris
    """

explain_plot_pairplot = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris
    """

# ---------------------
#   Multicollinearity
# ---------------------
explain_description_multicollinearity = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit
    """

explain_solution_multicollinearity = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit
    """

explain_plot_corrmatrix = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris
    """

# ---------------------
#      Normality
# ---------------------
explain_description_normality = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit
    """

explain_solution_normality = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris \
    nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit
    """

explain_plot_qq = """
    Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut \
    labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris
    """
